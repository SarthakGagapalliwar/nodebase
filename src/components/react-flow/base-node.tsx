...
export function BaseNode({ className, status, ...props }: BaseNodeProps) {
  return (
    <div
      className={cn(
        "bg-card relative border  border-muted-foreground rounded-sm text-card-foreground hover:bg-accent"        
       ,
        className,
      )}
      tabIndex={0}
    ...
    >
      ...
      {status === "error" && (
        ...
        status === "loading" &&(
          <Loader2Icon className="absolute -right-0.5 -bottom-0.5 size-2 text-blue-700 stroke-3
          animate-spin
          "/>
        )
      }
