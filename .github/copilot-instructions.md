...
- NIM Image execution node added (`NodeType.NIM_IMAGE`), registered in `node-components` with `/logos/nim.svg`. Uses NVIDIA FLUX API for image generation via `https://ai.api.nvidia.com/v1/genai/black-forest-labs/flux.1-dev`. Requires `NIM_API_KEY` env var. Supports Handlebars templating in prompt field (e.g., `{{myGemini.text}}`). Returns `imageUrl`, `imageBase64`, `prompt`, `width`, `height`.
- Dialog auto-save pattern: Use `form.watch()` subscription with debounce (1000ms default), call `form.trigger()` for validation before submit, use `isSavingRef` and `hasSavedRef` for visual feedback (LoaderIcon/CheckIcon).
- Editor auto-save pattern: Poll with interval (3000ms), compare snapshots via JSON stringify, use `lastSnapshotRef` for change detection, display CheckIcon on successful save.
- Google Sheets execution node added (`NodeType.GOOGLE_SHEETS`), registered in `node-components` with `/logos/google-sheets.svg`. Uses `googleapis` package with service account credentials (`CredentialType.GOOGLE_SHEETS`). Supports read, append, and write operations. Returns `rows`, `records`, `headers`, `rowCount` for read operations.
- Email execution node added (`NodeType.EMAIL`), registered in `node-components` with `/logos/resend.svg`. Uses `resend` package with API key credentials (`CredentialType.RESEND`). Supports dynamic recipients, subject, and body with Handlebars templating.
- Data Filter execution node added (`NodeType.DATA_FILTER`), registered in `node-components`. Filters arrays with top_percent, top_n, bottom_percent, bottom_n, or condition-based filtering. Great for "top 10% scorers" use cases. Returns `items`, `count`, `originalCount`, `filterType`.
